...
namespace proTools.proFolder {
    public class proFolderEditor : EditorWindow {
        ...
        private static Vector2 CalculateWindowSize() {
            var colorRowCount = Mathf.CeilToInt((_colors.Count + 1) / (float)MAX_COLUMNS);
            var markerRowCount = CalculateMarkerRowCount();
            var totalHeight = ((colorRowCount + markerRowCount) * ROW_HEIGHT) + BASE_HEIGHT;
        ...
        private static int CalculateMarkerRowCount() {
            ...
            var totalMarkerRows = 0;

            foreach (var group in groups) {
                var groupIconCount = group.Count();
                var iconRows = Mathf.CeilToInt(groupIconCount / (float)MAX_COLUMNS);
        ...
        private void DrawColorGrid() {
            if (_colorIcons == null || _markerLibrary == null) {
                return;
            }

            var totalItems = _colorIcons.Length + 1;
        ...
        private void DrawMarkerGrid() {
            if (_markerLibrary?.entries == null) {
                return;
            }
