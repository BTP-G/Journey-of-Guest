...
namespace proTools.proFolder {
    ...
    public class proFolderEditorWindow : EditorWindow {
        ...
        private void DrawSettings() {
            ...
            var currentIndex = data.enableOption ? 0 : 1;
            var selectedIndex = EditorGUILayout.Popup("Effect", currentIndex, new[] { "Enable", "Disable" });
            data.enableOption = selectedIndex == 0;

            var currentHotkeyIndex = data.hotkeyOption ? 0 : 1;
            var selectedHotkeyIndex = EditorGUILayout.Popup("Hotkey Mode", currentHotkeyIndex, new[] { "Shift + Click", "Alt + Click" });
        ...
        private void DrawColorPresets() {
            ...
            var scrollHeight = Mathf.Min((data.colorPresetList.Count * 25f) + 10f, 220f);
            scrollPosition = EditorGUILayout.BeginScrollView(scrollPosition, GUILayout.Height(scrollHeight));

            for (var i = 0; i < data.colorPresetList.Count; i++) {
