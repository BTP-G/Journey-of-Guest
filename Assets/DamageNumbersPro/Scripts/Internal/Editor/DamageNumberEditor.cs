    ...
    public class DamageNumberEditor : Editor {
        ...
        public static void SwitchToGUI(MenuCommand menuCommand) {
            foreach (var selected in Selection.gameObjects) {
                var oldComponent = selected.GetComponent<DamageNumberMesh>();
                ...
                if (oldComponent != null) {
                    ...
                    // Add GUI Component
                    var newComponent = oldComponent.gameObject.AddComponent<DamageNumberGUI>();
        ...
        public static void SwitchToMesh(MenuCommand menuCommand) {
            foreach (var selected in Selection.gameObjects) {
                var previousComponent = selected.GetComponent<DamageNumberGUI>();
                ...
                if (previousComponent != null) {
                    ...
                    // Add GUI Component
                    var newComponent = previousComponent.gameObject.AddComponent<DamageNumberMesh>();
                    UnityEditorInternal.ComponentUtility.MoveComponentUp(newComponent);
                    UnityEditorInternal.ComponentUtility.MoveComponentUp(newComponent);
                    UnityEditorInternal.ComponentUtility.MoveComponentUp(newComponent);
                    UnityEditorInternal.ComponentUtility.MoveComponentUp(newComponent);
            ...
            if (showProperties) {
                ...
                switch (featureTitle) {
                    ...
                    case "Rotate Over Time":
                        ...
                        var rotateOverTimeColor = Color.Lerp(Color.red, Color.green, rotateOverTimeProperty.animationCurveValue.Evaluate(1));
                    ...
                    case "Scale Over Time":
                        ...
                        var scaleOverTimeColor = Color.Lerp(Color.red, Color.green, scaleOverTimeProperty.animationCurveValue.Evaluate(1));
            ...
            if (textField.stringValue.Length > 5) {
                ...
                if (unicodeCheck && unicodeIndex < chars.Length - 5) {
                    if (GUILayout.Button("Unicode", GUILayout.Width(60))) {
                        // Get hex code
                        var unicodeHex = "";
                        for (var i = unicodeIndex + 2; i < unicodeIndex + 6; i++) {
                            unicodeHex += chars[i];
                        }
                        var hexCode = int.Parse(unicodeHex, System.Globalization.NumberStyles.HexNumber);
                        ...
                        // Combine
                        var newString = "";
                        for (var i = 0; i < unicodeIndex; i++) {
                            newString += chars[i];
                        }
                        newString += unicode;
                        for (var i = unicodeIndex + 6; i < chars.Length; i++) {
                            newString += chars[i];
                        }
        ...
        private bool FeatureButton(SerializedProperty property, string featureTitle) {
            ...
            var buttonName = property.hasMultipleDifferentValues ? "<b>− − −</b>" : (property.boolValue ? "<b> " + featureTitle + "</b>" : featureTitle);
        ...
        private void DisplayTextMain(bool isMesh) {
            ...
            EditorGUI.BeginChangeCheck();
            var newFontAsset = (TMP_FontAsset)EditorGUILayout.ObjectField(new GUIContent("Font", "The font used by text mesh pro."), fontAsset, typeof(TMP_FontAsset), false);
            ...
            EditorGUI.BeginChangeCheck();
            var newColor = EditorGUILayout.ColorField(new GUIContent("Color", "The vertex color used by text mesh pro."), vertexColor);
            if (EditorGUI.EndChangeCheck()) {
                foreach (var dn in DNPEditorInternal.damageNumbers) {
        ...
        private int TextPosition(int current) {
            ...
            var swapWith = GUILayout.Toolbar(current, new GUIContent[] { new GUIContent("←", "Move this text to the left."), new GUIContent("→", "Move this text to the right."), new GUIContent("↑", "Move this text to the top."), new GUIContent("↓", "Move this text to the bottom.") }, GUILayout.Width(DNPEditorInternal.currentWidth > 330 ? 90 : 0));
        ...
        private void DisplayDelay(string propertyName, string displayName, string displayTooltip) {
            ...
            if (property.hasMultipleDifferentValues) {
                ...
                var newFPS = EditorGUILayout.IntField(currentFPS, GUILayout.Width(DNPEditorInternal.currentWidth > 294 ? fpsWidth : 0));
