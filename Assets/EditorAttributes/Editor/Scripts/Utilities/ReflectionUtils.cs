...
namespace EditorAttributes.Editor.Utility {
    public static class ReflectionUtils {
        ...
        public static FieldInfo FindField(string fieldName, SerializedProperty property) {
            ...
            var fieldInfo = serializedObjectType != null ? serializedObjectType.GetField(fieldName, BINDING_FLAGS) : FindField(fieldName, property.serializedObject.targetObject);
        ...
        public static PropertyInfo FindProperty(string propertyName, SerializedProperty property) {
            ...
            var serializedObjectType = GetNestedObjectType(property, out _);
            var propertyInfo = serializedObjectType != null ? serializedObjectType.GetProperty(propertyName, BINDING_FLAGS) : FindProperty(propertyName, property.serializedObject.targetObject);

            return propertyInfo;
        }

        internal static PropertyInfo FindProperty(string propertyName, object targetObject) {
            return FindMember(propertyName, targetObject?.GetType(), BINDING_FLAGS, MemberTypes.Property) as PropertyInfo;
        }
