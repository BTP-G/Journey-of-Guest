...
namespace EditorAttributes.Editor {
    ...
    public class AnimatorParamDropdownDrawer : CollectionDisplayDrawer {
        ...
        public override VisualElement CreatePropertyGUI(SerializedProperty property) {
            ...
            var animatorParameters = GetAnimatorParameters(animatorParamAttribute, property, errorBox, out animatorParametersHash);
            var dropdownField = CreateDropdownField(animatorParameters, property);

            UpdateVisualElement(dropdownField, () => {
                var animatorParams = GetAnimatorParameters(animatorParamAttribute, property, errorBox, out animatorParametersHash);
        ...
        protected override void PasteValue(VisualElement element, SerializedProperty property, string clipboardValue) {
            ...
            var parameterName = clipboardValue;

            if (int.TryParse(clipboardValue, out var parameterHash) && animatorParametersHash.ContainsKey(parameterHash)) {
        ...
        private List<string> GetAnimatorParameters(AnimatorParamDropdownAttribute animatorParamAttribute, SerializedProperty property, HelpBox errorBox, out Dictionary<int, string> paramterHashTable) {
            ...
            var memberInfo = ReflectionUtils.GetValidMemberInfo(animatorParamAttribute.AnimatorFieldName, property);
