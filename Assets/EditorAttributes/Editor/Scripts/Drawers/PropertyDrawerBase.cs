...
namespace EditorAttributes.Editor {
    public class PropertyDrawerBase : PropertyDrawer {
            ...
            try {
                switch (property.propertyType) {
                    ...
                    case SerializedPropertyType.Color:
                        property.colorValue = ColorUtility.TryParseHtmlString(value, out var color) ? color : Color.white;
                        break;
        ...
        public static string GetSerializedPropertyName(string propertyName, SerializedProperty property) {
            return ReflectionUtils.GetValidMemberInfo(propertyName, property) is PropertyInfo ? $"<{propertyName}>k__BackingField" : propertyName;
        }
        ...
        protected void AddPropertyContextMenu(VisualElement element, SerializedProperty property) {
            ...
            element.AddManipulator(new ContextualMenuManipulator((@event) => {
                var searchText = $"h:#{property.serializedObject.targetObject.GetType().Name}.{property.propertyPath}={GetPropertyValueAsString(property).Replace(" ", "")}";
        ...
        private protected string CreatePropertySaveKey(SerializedProperty property, string key) {
            return $"{property.serializedObject.targetObject.GetInstanceID()}_{property.propertyPath}_{key}";
        }
#endif
        ...
        public static VisualElement CreateFieldForType(Type fieldType, string fieldName, object fieldValue, bool showMixedValue = false) {
            ...
            if (fieldType == typeof(string)) {
                ...
                foreach (var nestedField in nestedFields) {
                    var createdField = CreateFieldForType(nestedField.FieldType, nestedField.Name, nestedField.GetValue(fieldValue));
        ...
        public static void RegisterValueChangedCallbackByType<T>(VisualElement field, Action<object> valueCallback, object objectValue = null) {
