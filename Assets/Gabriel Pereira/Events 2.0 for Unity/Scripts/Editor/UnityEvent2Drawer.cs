    ...
    public class UnityEvent2Drawer : PropertyDrawer {
        ...
        private State GetState(SerializedProperty property) {
            ...
            if (state == null || state.m_ReorderableList.serializedProperty.serializedObject != property.serializedObject) {
                ...
                var listenersArray = property.FindPropertyRelative("m_PersistentCalls.m_Calls");
        ...
        private void DrawEventHeader(Rect rect) {
            var label = (string.IsNullOrEmpty(m_Text) ? "Event" : m_Text) + GetEventParams(m_DummyEvent);

            rect.width -= kButtonWidth;
            rect.height = EditorGUIUtility.singleLineHeight;

            var trashButtonRect = new Rect(rect.x + rect.width, rect.y, kButtonWidth, rect.height);
            var trashButton = EditorGUIUtility.IconContent("TreeEditor.Trash");
        ...
        private void DrawEventListener(Rect rect, int index, bool isActive, bool isFocused) {
            ...
            var callStateRect = subRects[0];
            var isStaticRect = subRects[1];
            var targetRect = subRects[2];
            var functionRect = subRects[3];
            var iconToolbarMinusRect = subRects[4];
            ...
            for (var i = 0; i < arguments.arraySize; i++) {
            ...
            if (arguments.arraySize > 0) {
                ...
                for (var i = 0; i < arguments.arraySize; i++) {
                    ...
                    if (argument.FindPropertyRelative(kIsDynamicArgument).boolValue) {
                        ...
                        for (var j = 0; j < dynamicArguments.arraySize; j++) {
        ...
        private UnityEventBase2 GetDummyEvent(SerializedProperty prop) {
            ...
            var tgtobj = prop.serializedObject.targetObject;
            if (tgtobj == null) {
                return new UnityEvent2();
            }

            var propPath = prop.propertyPath;
            var ft = tgtobj.GetType();
        ...
        private Rect[] GetRowRects(Rect rect) {
            ...
            var functionRect = new Rect(rect) {
                xMin = targetRect.xMax + kSpacing
            };
            functionRect.width -= m_IconToolbarMinusSize.x + kSpacing;

            var iconToolbarMinusRect = new Rect(functionRect) {
                xMin = functionRect.xMax + kSpacing,
                width = m_IconToolbarMinusSize.x
            };

            var argNameRect = new Rect(targetRect);
            ...
            var dynamicRect = new Rect(argNameRect) {
                xMin = argNameRect.xMax + kSpacing,
                width = kButtonWidth
            };

            var argRect = new Rect(rect) {
                xMin = targetRect.xMax + kSpacing
            };
        ...
        private void BuildArgumentsField(Rect rect, SerializedProperty listener) {
            ...
            var argNameRect = subRects[5];
            var dynamicCheckboxRect = subRects[6];
            var argRect = subRects[7];
            ...
            for (var i = 0; i < arguments.arraySize; i++) {
                ...
                if (modeEnum == PersistentListenerMode2.EventDefined) {
                    ...
                    var enumType = Type.GetType(assembly.stringValue, false);
                    var names = Enum.GetNames(enumType);

                    argument.intValue = EditorGUI.Popup(argRect, argument.intValue, names);
                } else if (modeEnum == PersistentListenerMode2.Char) {
                    var fieldRect = argRect;

                    fieldRect.width -= kButtonWidth;

                    EditorGUI.PropertyField(fieldRect, argument, GUIContent.none);

                    var resetButtonRect = new Rect(fieldRect.x + fieldRect.width, fieldRect.y, kButtonWidth, fieldRect.height);
                    var resetButton = EditorGUIUtility.IconContent("TreeEditor.Trash");
                    ...
                    if (argument.isExpanded) {
                        for (var j = 0; j < argument.arraySize; j++) {
                            if (desiredType.IsArray && desiredType.GetElementType().IsEnum) {
                                var names = Enum.GetNames(desiredType.GetElementType());
                                ...
                                var names = Enum.GetNames(desiredType.GetGenericArguments()[0]);
                                ...
                                var type = desiredType.IsArray ? desiredType.GetElementType() : desiredType.GetGenericArguments()[0];
        ...
        private float BuildDynamicArgumentField(SerializedProperty argument, Rect argumentRect) {
            var height = (EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing) * 2f;
            ...
            var isStaticRect = new Rect(argumentRect) {
                width = kButtonWidth
            };

            var targetRect = new Rect(argumentRect);
            targetRect.xMin += kButtonWidth + kSpacing;
            ...
            var dynamicArgRect = dynamicArgNameRect;
                ...
                for (var i = 0; i < arguments.arraySize; i++) {
                    ...
                    switch (modeEnum) {
                        ...
                        case PersistentListenerMode2.Enum:
                            ...
                            var enumNames = Enum.GetNames(desiredTypes[i]);
        ...
        private void GetMethodsForType(List<ValidMethodMap> methods, Type type, Type[] delegateArgumentsTypes = null, bool isDynamic = false, bool isGetter = false) {
            var newMethods = CalculateMethodMap(type, BindingFlags.Static | BindingFlags.Public, delegateArgumentsTypes, isDynamic, isGetter);
        ...
        private void GetMethodsForTargetAndMode(List<ValidMethodMap> methods, Object target, Type[] delegateArgumentsTypes = null, bool isDynamic = false, bool isGetter = false) {
            var newMethods = CalculateMethodMap(target, delegateArgumentsTypes, isDynamic, isGetter);
        ...
        private IEnumerable<ValidMethodMap> CalculateMethodMap(Type type, BindingFlags bindingFlags, Type[] types, bool isDynamic, bool isGetter) {
            ...
            foreach (var componentMethod in componentMethods) {
                ...
                if (isGetter) {
                    if (componentMethod.ReturnType != types[0]) {
                        continue;
                    }
                ...
                if (isDynamic) {
                    ...
                    for (var i = types.Length; i < componentParameters.Length; i++) {
                        modes[i] = GetMode(componentParameters[i].ParameterType);
                    }
        ...
        private void AddFunctionsForScript(GenericMenu menu, SerializedProperty listener, ValidMethodMap method, string targetName, Type[] delegateArgumentsTypes) {
            ...
            var path = GetFormattedMethodName(isStatic.boolValue, targetName, method.methodInfo, args, method.isDynamic, delegateArgumentsTypes);
        ...
        private bool CompareModes(SerializedProperty modes1, PersistentListenerMode2[] modes2) {
            ...
            for (var i = 0; i < modes1.arraySize; i++) {
        ...
        private PersistentListenerMode2 GetMode(SerializedProperty mode) {
            return mode.type switch {
                "bool" => PersistentListenerMode2.Bool,
                "float" => PersistentListenerMode2.Float,
                "int" => PersistentListenerMode2.Int,
                "string" => PersistentListenerMode2.String,
                "Enum" => (PersistentListenerMode2)mode.enumValueIndex,
                _ => PersistentListenerMode2.Void,
            };
        }
        ...
        private PersistentListenerMode2[] GetModes(ParameterInfo[] parameters) {
            ...
            var modes = new PersistentListenerMode2[parameters.Length];
        ...
        private string GetFormattedMethodName(bool isStatic, string targetName, MethodInfo method, string args, bool dynamic, Type[] delegateArgumentsTypes) {
            if (dynamic) {
                if (method.Name.StartsWith("get_") || method.Name.StartsWith("set_")) {
                    ...
                    for (var i = delegateArgumentsTypes.Length; i < count; i++) {
        ...
        private MethodInfo GetMethodInfo(SerializedProperty listenerTarget, SerializedProperty methodName, SerializedProperty arguments, SerializedProperty isStaticForInstance) {
            ...
            return UnityEventBase2.GetValidMethodInfo(listenerTarget.objectReferenceValue, methodName.stringValue, desiredTypes, isStaticForInstance != null && isStaticForInstance.boolValue);
        }
        ...
        private struct UnityEventFunction {
            ...
            public void Assign() {
                ...
                if (isStatic.boolValue && !m_Method.IsStatic) {
                    return;
                }
