...
namespace UnityEngine.Events {
    ...
    public abstract class UnityEvent2<T0, T1> : UnityEventBase2 {
        ...
        protected override MethodInfo FindMethod_Impl(object targetObj, string name, bool isStatic) {
            return GetValidMethodInfo(targetObj, name, new System.Type[2] { typeof(T0), typeof(T1) }, isStatic);
        }
        ...
        public void Invoke(T0 arg1, T1 arg2) {
            var calls = PrepareInvoke(arg1, arg2);
            for (var i = 0; i < calls.Count; i++) {
                if (calls[i] is InvokableCall2<T0, T1> curCall) {
