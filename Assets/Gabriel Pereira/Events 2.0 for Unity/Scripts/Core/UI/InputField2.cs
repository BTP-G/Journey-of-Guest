    ...
    public class InputField2
        ...
        public string text {
            ...
            set {
                ...
                if (onValidateInput != null || characterValidation != InputField.CharacterValidation.None) {
                    m_Text = "";
                    var validatorMethod = onValidateInput ?? Validate;
                    m_CaretPosition = m_CaretSelectPosition = value.Length;
                    var charactersToCheck = characterLimit > 0 ? Math.Min(characterLimit, value.Length) : value.Length;
                    for (var i = 0; i < charactersToCheck; ++i) {
                        var c = validatorMethod(m_Text, m_Text.Length, value[i]);
                        if (c != 0) {
                            m_Text += c;
                        }
                    }
                } else {
                    m_Text = characterLimit > 0 && value.Length > characterLimit ? value[..characterLimit] : value;
                }
        ...
        private IEnumerator CaretBlink() {
            ...
            while (isFocused && m_CaretBlinkRate > 0) {
                ...
                var blinkPeriod = 1f / m_CaretBlinkRate;

                // the caret should be ON if we are in the first half of the blink period
                var blinkState = (Time.unscaledTime - m_BlinkStartTime) % blinkPeriod < blinkPeriod / 2;
        ...
        public Vector2 ScreenToLocal(Vector2 screen) {
            ...
            if (theCanvas.renderMode == RenderMode.ScreenSpaceOverlay) {
                ...
                var mouseRay = theCanvas.worldCamera.ScreenPointToRay(screen);
                var plane = new Plane(m_TextComponent.transform.forward, m_TextComponent.transform.position);
        ...
        private int GetUnclampedCharacterLineFromPosition(Vector2 pos, TextGenerator generator) {
            ...
            var y = pos.y * m_TextComponent.pixelsPerUnit;
        ...
        protected int GetCharacterIndexFromPosition(Vector2 pos) {
            ...
            for (var i = startCharIndex; i < endCharIndex; i++) {
                ...
                var charPos = charInfo.cursorPos / m_TextComponent.pixelsPerUnit;

                var distToCharStart = pos.x - charPos.x;
                var distToCharEnd = charPos.x + (charInfo.charWidth / m_TextComponent.pixelsPerUnit) - pos.x;
        ...
        private IEnumerator MouseDragOutsideRect(PointerEventData eventData) {
            while (m_UpdateDrag && m_DragPositionOutOfBounds) {
                ...
                UpdateLabel();
                var delay = multiLine ? kVScrollSpeed : kHScrollSpeed;
        ...
        public override void OnPointerDown(PointerEventData eventData) {
            ...
            var hadFocusBefore = m_AllowInput;
        ...
        private void MoveDown(bool shift, bool goToLastChar) {
            ...
            var position = multiLine ? LineDownCharacterPosition(caretSelectPositionInternal, goToLastChar) : text.Length;
        ...
        private void MoveUp(bool shift, bool goToFirstChar) {
            ...
            var position = multiLine ? LineUpCharacterPosition(caretSelectPositionInternal, goToFirstChar) : 0;
        ...
        private void Delete() {
            ...
            if (caretPositionInternal < caretSelectPositionInternal) {
                m_Text = text[..caretPositionInternal] + text[caretSelectPositionInternal..];
                caretSelectPositionInternal = caretPositionInternal;
            } else {
                m_Text = text[..caretSelectPositionInternal] + text[caretPositionInternal..];
        ...
        protected virtual void Append(char input) {
            ...
            var insertionPoint = Math.Min(selectionFocusPosition, selectionAnchorPosition);
            if (onValidateInput != null) {
        ...
        protected void UpdateLabel() {
            if (m_TextComponent != null && m_TextComponent.font != null && !m_PreventFontCallback) {
                ...
                string fullText;
                if (compositionString.Length > 0) {
                    fullText = text[..m_CaretPosition] + compositionString + text[m_CaretPosition..];
                } else {
                    fullText = text;
                }
                ...
                if (!isEmpty) {
                    ...
                    var extents = m_TextComponent.rectTransform.rect.size;
                    ...
                    processed = processed[m_DrawStart..Mathf.Min(m_DrawEnd, processed.Length)];
        ...
        private void GenerateCaret(VertexHelper vbo, Vector2 roundingOffset) {
            ...
            var adjustedPos = Mathf.Max(0, caretPositionInternal - m_DrawStart);
            var gen = m_TextComponent.cachedTextGenerator;
            ...
            var height = gen.lines[characterLine].height / m_TextComponent.pixelsPerUnit;
            ...
            var displayIndex = m_TextComponent.canvas.targetDisplay;
        ...
        private void GenerateHightlight(VertexHelper vbo, Vector2 roundingOffset) {
            var startChar = Mathf.Max(0, caretPositionInternal - m_DrawStart);
            var endChar = Mathf.Max(0, caretSelectPositionInternal - m_DrawStart);

            // Ensure pos is always less then selPos to make the code simpler
            if (startChar > endChar) {
                (endChar, startChar) = (startChar, endChar);
            }
            ...
            while (currentChar <= endChar && currentChar < gen.characterCount) {
                if (currentChar == lastCharInLineIndex || currentChar == endChar) {
                    ...
                    var startPosition = new Vector2(startCharInfo.cursorPos.x / m_TextComponent.pixelsPerUnit, gen.lines[currentLineIndex].topY / m_TextComponent.pixelsPerUnit);
                    var endPosition = new Vector2((endCharInfo.cursorPos.x + endCharInfo.charWidth) / m_TextComponent.pixelsPerUnit, startPosition.y - (gen.lines[currentLineIndex].height / m_TextComponent.pixelsPerUnit));
        ...
        protected char Validate(string text, int pos, char ch) {
            ...
            if (characterValidation is InputField.CharacterValidation.Integer or InputField.CharacterValidation.Decimal) {
                // Integer and decimal
                var cursorBeforeDash = pos == 0 && text.Length > 0 && text[0] == '-';
                var dashInSelection = text.Length > 0 && text[0] == '-' && ((caretPositionInternal == 0 && caretSelectPositionInternal > 0) || (caretSelectPositionInternal == 0 && caretPositionInternal > 0));
                var selectionAtStart = caretPositionInternal == 0 || caretSelectPositionInternal == 0;
                if (!cursorBeforeDash || dashInSelection) {
                    ...
                    if (ch == '.' && characterValidation == InputField.CharacterValidation.Decimal && !text.Contains(".")) {
                        return ch;
                    }
                }
