    ...
    public class Dropdown2 : Selectable, IPointerClickHandler, ISubmitHandler, ICancelHandler {
        protected class DropdownItem : MonoBehaviour, IPointerEnterHandler, ICancelHandler {
            ...
            public virtual void OnCancel(BaseEventData eventData) {
                var dropdown = GetComponentInParent<Dropdown2>();
                if (dropdown) {
                    dropdown.Hide();
                }
            }
        ...
        private void SetupTemplate() {
            ...
            var itemToggle = m_Template.GetComponentInChildren<Toggle2>();

            validTemplate = true;
            if (!itemToggle || itemToggle.transform == template) {
                validTemplate = false;
                Debug.LogError("The dropdown template is not valid. The template must have a child GameObject with a Toggle component serving as the item.", template);
            } else if (itemToggle.transform.parent is not RectTransform) {
                validTemplate = false;
        ...
        public void Show() {
            ...
            for (var i = 0; i < options.Count; ++i) {
                var data = options[i];
                var item = AddItem(data, value == i, itemTemplate, m_Items);
                if (item == null) {
                    continue;
                }
