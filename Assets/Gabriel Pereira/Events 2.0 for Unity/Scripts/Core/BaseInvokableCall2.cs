...
namespace UnityEngine.Events {
    public abstract class BaseInvokableCall2 {
        ...
        protected object[] HandlePersistentCalls(PersistentCall2[] persistentCalls, object[] args) {
            var result = new object[persistentCalls.Length];

            for (var i = 0; i < persistentCalls.Length; i++) {
                var persistentCall = persistentCalls[i];

                if (persistentCall != null) {
                    var argumentTypes = new Type[persistentCall.arguments.Count];
                    var parameters = new object[persistentCall.arguments.Count];
