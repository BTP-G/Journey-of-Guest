...
namespace ANU.IngameDebug.Console {
    ...
    public class DebugConsole : MonoBehaviour {
        ...
        public static ExecutionResult ExecuteCommand(string commandLine, bool silent = false) {
            return _processor.ExecuteCommand(commandLine, silent);
        }
        ...
        private void Start() {
            GetComponentInChildren<UICanvasScaler>(includeInactive: true).RefreshConsoleScale();
        }
        ...
        private void HandleLogs() {
            while (_logs.TryDequeue(out var log)) {
                ...
                if (match.Success && match.Index == 0) {
                    log.condition = log.condition[match.Length..];
