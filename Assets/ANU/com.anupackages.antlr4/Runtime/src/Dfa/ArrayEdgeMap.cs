...
namespace Antlr4.Runtime.Dfa {
    ...
    public sealed class ArrayEdgeMap<T> : AbstractEdgeMap<T>
        ...
        public override AbstractEdgeMap<T> Put(int key, T value) {
            if (key >= minIndex && key <= maxIndex) {
                var existing = Interlocked.Exchange(ref arrayData[key - minIndex], value);
        ...
        public override AbstractEdgeMap<T> PutAll(IEdgeMap<T> m) {
            ...
            if (m is Antlr4.Runtime.Dfa.ArrayEdgeMap<T>) {
                var other = (Antlr4.Runtime.Dfa.ArrayEdgeMap<T>)m;
                var minOverlap = Math.Max(minIndex, other.minIndex);
                var maxOverlap = Math.Min(maxIndex, other.maxIndex);
                var result = this;
                for (var i = minOverlap; i <= maxOverlap; i++) {
