    ...
    public class ProfilingATNSimulator : ParserATNSimulator {
        ...
        public override int AdaptivePredict(ITokenStream input, int decision, ParserRuleContext outerContext) {
            try {
                ...
                var start = DateTime.Now.ToFileTime(); // expensive but useful info
                var alt = base.AdaptivePredict(input, decision, outerContext);
                var stop = DateTime.Now.ToFileTime();
                decisions[decision].timeInPrediction += stop - start;
                decisions[decision].invocations++;

                var SLL_k = sllStopIndex - startIndex + 1;
                ...
                if (llStopIndex >= 0) {
                    var LL_k = llStopIndex - startIndex + 1;
        ...
        protected override bool EvalSemanticContext(SemanticContext pred, ParserRuleContext parserCallStack, int alt, bool fullCtx) {
            var result = base.EvalSemanticContext(pred, parserCallStack, alt, fullCtx);
            if (pred is not SemanticContext.PrecedencePredicate) {
                var fullContext = llStopIndex >= 0;
                var stopIndex = fullContext ? llStopIndex : sllStopIndex;
