    ...
    public class ATNDeserializer {
        ...
        protected internal virtual void OptimizeATN(ATN atn) {
            while (true) {
                ...
                var preserveOrder = atn.grammarType == ATNType.Lexer;
        ...
        protected internal virtual void GenerateRuleBypassTransitions(ATN atn) {
            ...
            for (var i_10 = 0; i_10 < atn.ruleToStartState.Length; i_10++) {
                atn.ruleToTokenType[i_10] = atn.maxTokenType + i_10 + 1;
            }
            for (var i_13 = 0; i_13 < atn.ruleToStartState.Length; i_13++) {
                ...
                if (atn.ruleToStartState[i_13].isPrecedenceRule) {
                    ...
                    endState = null;
                    foreach (var state_3 in atn.states) {
                        if (state_3.ruleIndex != i_13) {
                            continue;
                        }
                        if (state_3 is not StarLoopEntryState) {
                            continue;
                        }
                        var maybeLoopEndState = state_3.Transition(state_3.NumberOfTransitions - 1).target;
                ...
                foreach (var state_4 in atn.states) {
                    foreach (var transition in state_4.transitions) {
                ...
                while (atn.ruleToStartState[i_13].NumberOfTransitions > 0) {
                    var transition = atn.ruleToStartState[i_13].Transition(atn.ruleToStartState[i_13].NumberOfTransitions - 1);
        ...
        protected internal virtual void ReadLexerActions(ATN atn) {
            ...
            if (atn.grammarType == ATNType.Lexer) {
                ...
                for (var i_10 = 0; i_10 < atn.lexerActions.Length; i_10++) {
                    var actionType = (LexerActionType)ReadInt();
                    var data1 = ReadInt();
                    var data2 = ReadInt();
        ...
        protected internal virtual void ReadDecisions(ATN atn) {
            ...
            // DECISIONS
            //
            var ndecisions = ReadInt();
            for (var i_11 = 0; i_11 < ndecisions; i_11++) {
                var s = ReadInt();
            ...
            for (var i_12 = 0; i_12 < ndecisions; i_12++) {
        ...
        protected internal virtual void ReadEdges(ATN atn, IList<IntervalSet> sets) {
            ...
            //
            var nedges = ReadInt();
            for (var i_9 = 0; i_9 < nedges; i_9++) {
                var src = ReadInt();
                var trg = ReadInt();
                var ttype = (TransitionType)ReadInt();
                var arg1 = ReadInt();
                var arg2 = ReadInt();
                var arg3 = ReadInt();
            ...
            foreach (var state_1 in atn.states) {
                for (var i_10 = 0; i_10 < state_1.NumberOfTransitions; i_10++) {
                    var t = state_1.Transition(i_10);
            ...
            foreach (var state_2 in atn.states) {
                if (state_2 is BlockStartState) {
                    ...
                    var loopbackState = (PlusLoopbackState)state_2;
                    for (var i_10 = 0; i_10 < loopbackState.NumberOfTransitions; i_10++) {
                        var target = loopbackState.Transition(i_10).target;
        ...
        protected internal virtual void ReadSets(ATN atn, IList<IntervalSet> sets) {
            ...
            //
            var nsets = ReadInt();
            for (var i_8 = 0; i_8 < nsets; i_8++) {
                var set = new IntervalSet();
                sets.Add(set);
                var nintervals = ReadInt();
                var containsEof = ReadInt() != 0;
                if (containsEof) {
                    set.Add(-1);
                }
        ...
        protected internal virtual void ReadRules(ATN atn) {
            ...
            foreach (var state in atn.states) {
                ...
                var stopState = (RuleStopState)state;
        ...
        protected internal virtual void ReadStates(ATN atn) {
            ...
            var nstates = ReadInt();
            for (var i_1 = 0; i_1 < nstates; i_1++) {
                var stype = (StateType)ReadInt();
            ...
            foreach (var pair_1 in endStateNumbers) {
                pair_1.Item1.endState = (BlockEndState)atn.states[pair_1.Item2];
            }
            var numNonGreedyStates = ReadInt();
            for (var i_2 = 0; i_2 < numNonGreedyStates; i_2++) {
                var stateNumber = ReadInt();
                ((DecisionState)atn.states[stateNumber]).nonGreedy = true;
            }
            var numPrecedenceStates = ReadInt();
            for (var i_4 = 0; i_4 < numPrecedenceStates; i_4++) {
                var stateNumber = ReadInt();
        ...
        protected internal virtual ATN ReadATN() {
            var grammarType = (ATNType)ReadInt();
            var maxTokenType = ReadInt();
        ...
        protected internal virtual void CheckVersion() {
            ...
            if (version != SerializedVersion) {
                var reason = string.Format(CultureInfo.CurrentCulture, "Could not deserialize ATN with version {0} (expected {1}).", version, SerializedVersion);
        ...
        private static int InlineSetRules(ATN atn) {
            var inlinedCalls = 0;
            var ruleToInlineTransition = new Transition[atn.ruleToStartState.Length];
            for (var i = 0; i < atn.ruleToStartState.Length; i++) {
                ...
                if (matchTransition.IsEpsilon || !matchTarget.OnlyHasEpsilonTransitions || matchTarget.NumberOfOptimizedTransitions != 1 || matchTarget.GetOptimizedTransition(0).target is not RuleStopState) {
                    continue;
                }
            ...
            for (var stateNumber = 0; stateNumber < atn.states.Count; stateNumber++) {
                var state = atn.states[stateNumber];
                if (state.ruleIndex < 0) {
                    continue;
                }
                IList<Transition> optimizedTransitions = null;
                for (var i_1 = 0; i_1 < state.NumberOfOptimizedTransitions; i_1++) {
                    var transition = state.GetOptimizedTransition(i_1);
                    if (transition is not RuleTransition) {
                ...
                if (optimizedTransitions != null) {
                    ...
                    foreach (var transition in optimizedTransitions) {
        ...
        private static int CombineChainedEpsilons(ATN atn) {
            ...
            foreach (var state in atn.states) {
                ...
                for (var i = 0; i < state.NumberOfOptimizedTransitions; i++) {
                    ...
                    for (var j_2 = 0; j_2 < intermediate.NumberOfOptimizedTransitions; j_2++) {
                        var target = intermediate.GetOptimizedTransition(j_2).target;
        ...
        private static int OptimizeSets(ATN atn, bool preserveOrder) {
            ...
            foreach (var decision in decisions) {
                ...
                for (var i = 0; i < decision.NumberOfOptimizedTransitions; i++) {
                    ...
                    if (transition is AtomTransition or RangeTransition or SetTransition) {
                ...
                for (var i_1 = 0; i_1 < decision.NumberOfOptimizedTransitions; i_1++) {
                ...
                for (var i_2 = 0; i_2 < setTransitions.GetIntervals().Count; i_2++) {
                    var interval = setTransitions.GetIntervals()[i_2];
                ...
                foreach (var transition_1 in optimizedTransitions) {
        ...
        private static void IdentifyTailCalls(ATN atn) {
            foreach (var state in atn.states) {
                ...
                foreach (var transition_1 in state.optimizedTransitions) {
                    if (transition_1 is not RuleTransition) {
                        continue;
                    }
                    var ruleTransition = (RuleTransition)transition_1;
                    ruleTransition.tailCall = TestTailCall(atn, ruleTransition, false);
                    ruleTransition.optimizedTailCall = TestTailCall(atn, ruleTransition, true);
                }
            }
        ...
        private static bool TestTailCall(ATN atn, RuleTransition transition, bool optimizedPath) {
            ...
            var reachable = new BitSet(atn.states.Count);
            var worklist = new Stack<ATNState>();
        ...
        protected internal virtual Transition EdgeFactory(ATN atn, TransitionType type, int src, int trg, int arg1, int arg2, int arg3, IList<IntervalSet> sets) {
            ...
            switch (type) {
                ...
                case TransitionType.RULE: {
                    var rt = new RuleTransition((RuleStartState)atn.states[arg1], arg2, arg3, target);
                    return rt;
                }
        ...
        protected internal virtual ATNState StateFactory(StateType type, int ruleIndex) {
            ...
            switch (type) {
                ...
                default: {
                    var message = string.Format(CultureInfo.CurrentCulture, "The specified state type {0} is not valid.", type);
        ...
        protected internal virtual ILexerAction LexerActionFactory(LexerActionType type, int data1, int data2) {
            switch (type) {
                ...
                default: {
                    var message = string.Format(CultureInfo.CurrentCulture, "The specified lexer action type {0} is not valid.", type);
                    throw new ArgumentException(message);
                }
            }
        }
