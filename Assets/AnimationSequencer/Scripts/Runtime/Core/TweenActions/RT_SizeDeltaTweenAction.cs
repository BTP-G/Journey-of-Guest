...
namespace BrunoMikoski.AnimationSequencer {
    ...
    public sealed class RT_SizeDeltaTweenAction : TweenActionBase {
        ...
        protected override Tweener GenerateTween_Internal(GameObject target, float duration) {
            ...
            var endValue = toPercentageMode ? Vector2.Scale(targetRectTransform.rect.size, toSizeDelta / 100) : toSizeDelta;
            if (IsRectTransformStretched(targetRectTransform, out var isHorizontallyStretched, out var isVerticallyStretched)) {
                var strechValue = -(targetRectTransform.rect.size - (endValue + targetRectTransform.sizeDelta));
                if (isHorizontallyStretched) {
                    endValue.x = strechValue.x;
                }

                if (isVerticallyStretched) {
                    endValue.y = strechValue.y;
                }
            }
        ...
        private bool IsRectTransformStretched(RectTransform rectTransform, out bool isHorizontallyStretched, out bool isVerticallyStretched) {
            ...
            static bool IsValueInSet(float value) {
                return value is 0f or 0.5f or 1f;
            }
        }
