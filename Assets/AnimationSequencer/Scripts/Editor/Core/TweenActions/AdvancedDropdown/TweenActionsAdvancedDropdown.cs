...
namespace BrunoMikoski.AnimationSequencer {
    ...
    public sealed class TweenActionsAdvancedDropdown : AdvancedDropdown {
        ...
        protected override AdvancedDropdownItem BuildRoot() {
            var root = new AdvancedDropdownItem("Tween Actions");

            foreach (var typeToDisplayGUI in AnimationSequencerEditorGUIUtility.TypeToDisplayName) {
                var baseTweenActionType = typeToDisplayGUI.Key;

                var targetFolder = root;

                if (AnimationSequencerEditorGUIUtility.TypeToParentDisplay.TryGetValue(baseTweenActionType, out var parent)) {
                    var item = targetFolder.children.FirstOrDefault(dropdownItem =>
                ...
                var tweenActionAdvancedDropdownItem =
        ...
        onActionSelectedCallback) {
            ...
            actionsList = actionsListSerializedProperty;
        ...
        public bool IsTypeAlreadyInUse(SerializedProperty actionsSerializedProperty, Type targetType) {
            ...
            for (var i = 0; i < actionsSerializedProperty.arraySize; i++) {
                var actionElement = actionsSerializedProperty.GetArrayElementAtIndex(i);
