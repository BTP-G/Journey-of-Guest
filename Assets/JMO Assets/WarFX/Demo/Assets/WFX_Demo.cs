...
public class WFX_Demo : MonoBehaviour {
    ...
    private readonly string randomSpawnsDelay = "0.5";
    private readonly bool randomSpawns;
    ...
    private readonly List<string> groundTextures = new List<string>(new string[] { "Concrete", "Wood", "Metal", "Checker" });
    ...
    public GameObject spawnParticle() {
        var particles = Instantiate(ParticleExamples[exampleIndex]);
    ...
    private void OnGUI() {
		...
		*/
        ...
        if (m4.GetComponent<Renderer>().enabled) {
            ...
            var rx = m4.transform.localEulerAngles.x;
            rx = rx > 90 ? rx - 180 : rx;
            var ry = m4.transform.localEulerAngles.y;
            var rz = m4.transform.localEulerAngles.z;
    ...
    LOOP:
        ...
        if (orderedSpawns) {
            ...
            particles.transform.position = transform.position + new Vector3(Random.Range(-range, range), 0, Random.Range(-range, range)) + new Vector3(0, particles.transform.position.y, 0);
        }
    ...
    private void prevTexture() {
        var index = groundTextures.IndexOf(groundTextureStr);
        index--;
        if (index < 0) {
    ...
    private void showHideStuff() {
        ...
        if (ParticleExamples[exampleIndex].name.StartsWith("WFX_BImpact")) {
            ...
            var rs = bulletholes.GetComponentsInChildren<Renderer>();
            foreach (var r in rs) {
                r.enabled = true;
            }
        } else {
