...
public class CFX_SpawnSystemEditor : Editor {
    ...
    private SerializedProperty p_hideObjectsInHierarchy {
        get {
            _hideObjectsInHierarchy ??= serializedObject.FindProperty("hideObjectsInHierarchy");
    ...
    private SerializedProperty p_onlyGetInactiveObjects {
        get {
            _onlyGetInactiveObjects ??= serializedObject.FindProperty("onlyGetInactiveObjects");
    ...
    private SerializedProperty p_instantiateIfNeeded {
        get {
            _instantiateIfNeeded ??= serializedObject.FindProperty("instantiateIfNeeded");
    ...
    private SerializedProperty p_spawnAsChildren {
        get {
            _spawnAsChildren ??= serializedObject.FindProperty("spawnAsChildren");
    ...
    public override void OnInspectorGUI() {
        ...
        for (var i = 0; i < (target as CFX_SpawnSystem).objectsToPreload.Length; i++) {
            GUILayout.BeginHorizontal();

            EditorGUI.BeginChangeCheck();
            var obj = (GameObject)EditorGUILayout.ObjectField((target as CFX_SpawnSystem).objectsToPreload[i], typeof(GameObject), true);
            if (EditorGUI.EndChangeCheck()) {
#if UNITY_4_2
...
#endif
                (target as CFX_SpawnSystem).objectsToPreload[i] = obj;
            }
            ...
            EditorGUI.BeginChangeCheck();
            var nb = EditorGUILayout.IntField("", (target as CFX_SpawnSystem).objectsToPreloadTimes[i], GUILayout.Width(50));
            if (nb < 1) {
                nb = 1;
            }
...
#else
                Undo.RecordObject(target, "change Spawn System preload durationCount");
#endif
                (target as CFX_SpawnSystem).objectsToPreloadTimes[i] = nb;
            }
            ...
            if (GUILayout.Button("X", EditorStyles.miniButton, GUILayout.Width(24))) {
                Object preloadedObject = (target as CFX_SpawnSystem).objectsToPreload[i];
                var objectName = (preloadedObject == null) ? "" : preloadedObject.name;
#if UNITY_4_2
...
#endif
                ArrayUtility.RemoveAt<GameObject>(ref (target as CFX_SpawnSystem).objectsToPreload, i);
                ArrayUtility.RemoveAt<int>(ref (target as CFX_SpawnSystem).objectsToPreloadTimes, i);
        ...
        if ((Event.current.type == EventType.DragPerform || Event.current.type == EventType.DragUpdated)
            ...
            if (Event.current.type == EventType.DragPerform) {
                foreach (var o in DragAndDrop.objectReferences) {
                    if (o is GameObject) {
                        var already = false;
                        foreach (var otherObj in (target as CFX_SpawnSystem).objectsToPreload) {
                            if (o == otherObj) {
                                already = true;
...
#endif
                            ArrayUtility.Add<GameObject>(ref (target as CFX_SpawnSystem).objectsToPreload, (GameObject)o);
                            ArrayUtility.Add<int>(ref (target as CFX_SpawnSystem).objectsToPreloadTimes, 1);
