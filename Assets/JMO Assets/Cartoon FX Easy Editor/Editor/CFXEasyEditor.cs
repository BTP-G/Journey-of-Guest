...
public class CFXEasyEditor : EditorWindow {
    ...
    private static void ShowWindow() {
        var window = EditorWindow.GetWindow<CFXEasyEditor>(EditorPrefs.GetBool("CFX_ShowAsToolbox", true), "Easy Editor", true);
    ...
    private class ParticleSystemModule {
        ...
        public void CheckIfEnabledInSource(SerializedObject serializedSource) {
            ...
            if (sp != null) {
                ...
                if (enabled != null) {
                    enabledInSource = enabled.boolValue;
                }
            }
        }
    }
    ...
    private void OnGUI() {
        ...
        using (new EditorGUI.DisabledScope(SelectedParticleSystemsCount <= 0)) {
            ...
            if (GUILayout.Button("ClearAllBuffs", EditorStyles.miniButtonRight, GUILayout.Width(50f))) {
                foreach (var go in Selection.gameObjects) {
                    var systems = go.GetComponents<ParticleSystem>();
                    if (systems.Length == 0) {
                        continue;
        ...
        if (basicFoldout) {
            ...
            GUILayout.BeginHorizontal();
            if (GUILayout.Button(new GUIContent("Set Duration", "Changes the durationCount of the Particle System(s)"), GUILayout.Width(120))) {
                applyDuration();
            }
            ...
            if (DurationValue < 0.1f) {
                DurationValue = 0.1f;
            } else if (DurationValue > 9999) {
                DurationValue = 9999;
            }
        ...
        if (copyFoldout) {
            ...
            if (GUILayout.Button("Copy properties to selected Object(s)")) {
                ...
                foreach (var go in Selection.gameObjects) {
                    ...
                    foundPs = true;
                    foreach (var system in systems) {
                        CopyModules(sourceObject, system);
                    }
                }
    ...
    private void CopyModules(ParticleSystem source, ParticleSystem dest) {
        ...
        if (modulesToCopy[11].selected) {
            GenericModuleCopy(psSource.FindProperty("RotationBySpeedModule"), psDest.FindProperty("RotationBySpeedModule"));
        }
        ...
        if (modulesToCopy[13].selected) {
            SubModuleCopy(psSource, psDest);
        }
        ...
        if (modulesToCopy[16].selected) {
            GenericModuleCopy(psSource.FindProperty("InheritVelocityModule"), psDest.FindProperty("InheritVelocityModule"));
        }
