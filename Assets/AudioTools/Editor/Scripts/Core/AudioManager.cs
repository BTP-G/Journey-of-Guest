    ...
    public static class AudioManager {
        ...
        public static async Task<AudioClip> LoadAudioFromFile(string filePath, bool allowStreaming = true) {
            ...
            if (filePath.Contains("#") || filePath.Contains("+") || filePath.IsUnicode()) {
                var newName = Path.Combine(Application.temporaryCachePath, "AudioToolPreview" + Path.GetExtension(filePath));
            ...
            var fallbackChain = Path.GetExtension(filePath).ToLowerInvariant() switch {
                ".aiff" or ".aif" => new List<AudioType> { AudioType.AIFF, AudioType.OGGVORBIS, AudioType.WAV, AudioType.UNKNOWN },
                ".ogg" => new List<AudioType> { AudioType.OGGVORBIS, AudioType.WAV, AudioType.UNKNOWN, AudioType.AIFF },
                ".wav" => new List<AudioType> { AudioType.WAV, AudioType.OGGVORBIS, AudioType.UNKNOWN, AudioType.AIFF },
                ".mp3" => new List<AudioType> { AudioType.MPEG, AudioType.WAV, AudioType.UNKNOWN, AudioType.AIFF },
                _ => new List<AudioType> { AudioType.UNKNOWN, AudioType.OGGVORBIS, AudioType.WAV, AudioType.AIFF },
            };
            ...
            foreach (var type in fallbackChain) {
                using var uwr = UnityWebRequestMultimedia.GetAudioClip(fileUri, type);
                ...
                var shouldStream = allowStreaming && (type == AudioType.OGGVORBIS ||
                    type == AudioType.MPEG ||
                    type == AudioType.WAV ||
                    type == AudioType.AIFF);
                ...
                if (dlHandler.isDone) {
                    ...
                    if (clip == null || (clip.channels == 0 && clip.length == 0)) {
                        continue;
                    }
...
#if !AUDIO_TOOL_NOAUDIO
            if (EditorAudioUtility.IsPreviewClipPlaying() && EditorAudioUtility.LastPlayedPreviewClip != null) {
                var clip = EditorAudioUtility.LastPlayedPreviewClip;
                var position = EditorAudioUtility.GetPreviewClipPosition();
        ...
        public static bool HasReachedRangeEnd() {
            ...
            if (!EditorAudioUtility.IsPreviewClipPlaying()) {
                return true;
            }

            var currentSample = GetCurrentSamplePosition();
