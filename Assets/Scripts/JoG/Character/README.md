...
- 实体契约位于 `Packages/io.github.xoderony.jog/Runtime/Entities`：`Entity` 为每个实体创建 VContainer 子作用域，注册子 GameObject 上的 `IComponent` 以及 `Entity.Components` 中的 SerializeReference 组件，并转发生命周期、权威和同步事件。
