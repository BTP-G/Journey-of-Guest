...
- 通用契约位于 `Packages/io.github.xoderony.gameplay-effects`，只定义 `GameplayEffectData`、Definition、Controller 和全局注册表，不依赖 JoG、NGO 或 VContainer。
...
- 旧 `Assets/Scripts/JoG/Buff` 实现已迁移，本目录和 `CharacterEffects` 是当前入口。
