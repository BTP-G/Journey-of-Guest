...
namespace Expriverse.Localization {
    ...
    public class LanguageDropdown : MonoBehaviour {
        ...
        private void OnEnable() {
            languageDropdown.SetValueWithoutNotify(LanguageCodeToIndex(Localizer.LanguageCode));
        }

        private void OnLanguageSelected(int index) {
            Localizer.LanguageCode = IndexToLanguageCode(index);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private string IndexToLanguageCode(int index) {
            return index switch {
                0 => "zh-CN",
                _ => "en-US"
            };
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private int LanguageCodeToIndex(string languageCode) {
