...
本目录是 Expriverse 的项目侧实现，统一编入 `Assembly-CSharp`。本文记录跨模块事实；角色、效果、网络等细节见各子目录 `README.md`。
...
- `Assets/Scripts/Expriverse`：项目具体实现，命名空间使用 `Expriverse.*`，无 asmdef，编入 `Assembly-CSharp`。
- `Packages/io.github.xoderony.expriverse`：可供独立 Mod 引用的稳定 Expriverse 契约，程序集 `Expriverse`，不得反向依赖 `Assembly-CSharp`。
- `Packages/io.github.xoderony.*`：跨项目基础设施，程序集使用 `Xoderony.*`。
- 固定依赖方向为 `Assembly-CSharp` / 外部 Mod → `Expriverse` → `Xoderony.*`。具体 asmdef 引用以各包配置为准。
...
| 包 | 主要职责 |
| --- | --- |
| `io.github.xoderony.expriverse` | Entity/IComponent、Stat、Health、StateMachines、Interaction、Faction、Modding、旧消息 Broker、序列化契约 |
...
| `io.github.xoderony.gameplay-effects` | 与 Expriverse/NGO/VContainer 解耦的效果 Data/Definition/Controller 契约与注册表 |
...
- 2026-08-10 已移除零引用第三方资源包和演示场景，备份位于仓库外 `E:\UnityProjects\Expriverse Backup\2026-08-10-redundant`。
- `GameplayScene_2` 烘焙光照贴图改为本地生成并被 `.gitignore` 忽略。
- 同日使用 `git filter-repo` 清理历史并 force push，pack 从约 3.34 GiB 降至 0.47 GiB；旧克隆需要重新同步。
- 2026-08-17：`Xoderony.Networking` 与项目侧 `Expriverse.Networking.P2P`（`Assets/Scripts/Networking`）已落地会话/消息/对象 id 分配与 NV/RPC 模块，玩法仍走 NGO；P2P 未接线、未做 Unity 验证。`PROJECT_CONTEXT.md` 已拆为 `PROJECT_INDEX.md` + 各模块 README。
- 2026-08-23：RootScope、场景切换、场景级 Popup 和 P2P Root Runtime 已完成源码接线；Unity 编译、Prefab/场景序列化和双 Steam 客户端仍待验证。
- 当前占位实现包括 `UI/IngameOverlayController.cs`、`ExpriverseApplication.Initialize`。
- `Assets/Scripts/Expriverse/Buff` 只剩旧目录元数据；实现已迁到 GameplayEffects/CharacterEffects。
