...
namespace RealToon.Editor.Welcome {
    ...
    public class RTWelcome : EditorWindow {
        ...
        private static string CurrRP;
        private static string CurrRPDisp;
        private static string CurrRPVer;
        private static string CurrRPFullVer;
        private static string UniVer;
        private static string UniVerFull = null;
        private static string VRC_LV_Stat;
        private static bool VRC_LV_CHK;
        private static bool SRP_CHK_AVAI;
        private static Object RT_Pack;
        private static string RT_pac_name;
        private static Object RP_ReMe;
        private static Object RP_Qui_ReMe;
        private static Vector2 scroll;
        private static Vector2 scroll2;
        private static Vector2 scroll3;
        private static string rt_welcome_settings = "Assets/RealToon/Editor/RTW.sett";
        private static string[] RP_Info = { "Built-In Render Pipeline (BiRP)",
        ...
        private static void check_srp_ver() {
        ...
        #region check_vrc_lv
        private static void check_vrc_lv() {
        ...
        #region rt_pac_set
        private static void rt_pac_set() {
            if (GraphicsSettings.currentRenderPipeline != null) {
                if (CurrRP is "Universal Render Pipeline" or "Universal RP") {
                    if (int.Parse(CurrRPVer) is 12 or 13) {
                        RT_pac_name = "RealToon URP (URP 15 and 16)";
                    }
                    if (int.Parse(CurrRPVer) == 14) {
                        RT_pac_name = "RealToon URP (URP 14)";
                    }
                    if (int.Parse(CurrRPVer) is 15 or 16) {
                        RT_pac_name = "RealToon URP (URP 15 and 16)";
                    }

                    if (UniVerFull[..6] == "6000.0") {
                        RT_pac_name = "RealToon URP (Unity 6.0)";
                    } else if (UniVerFull[..6] == "6000.1" || (int.Parse(UniVerFull[..1]) == 6 && int.Parse(UniVerFull.Substring(5, 1)) >= 1)) {
                ...
                } else if (CurrRP is "High Definition Render Pipeline" or "High Definition RP") {

                    if (int.Parse(CurrRPVer) is 12 or 13) {
                        RT_pac_name = "RealToon HDRP (HDRP 12 and 13)";
                    } else if (int.Parse(CurrRPVer) == 16) {
                        RT_pac_name = "RealToon HDRP (HDRP 16)";
                    } else if (int.Parse(CurrRPVer) is 14 or 15) {
                ...
                if (int.Parse(Application.unityVersion[..4]) >= 2019) {
                    RT_pac_name = "RealToon Built-In RP [3D] (Unity 2019 and Later)";
                } else if (int.Parse(Application.unityVersion[..4]) < 2019) {
        ...
        #region Process & UI
        private void OnGUI() {
            var t = EditorGUIUtility.Load("Assets/RealToon/Editor/RT_GUI_Img.png") as Texture2D;
            var rect = new Rect(233, 0, t.width + 70, t.height + 70);
            ...
            var centeredtext = GUI.skin.GetStyle("Label");
            ...
            var Pro_info = GUI.skin.GetStyle("Label");
            ...
            var Pro_info2 = GUI.skin.GetStyle("Label");
        ...
        private void OnEnable() {
            check_srp_ver();
            check_vrc_lv();
            UniVer = Application.unityVersion[..4];
