...
- 2026-08-14 对象复制：`INetworkObjectManager` / `NetworkObjectManager` 负责会话内网络对象的生成/销毁/查找、状态/RPC 投递、晚加入补发与离开清理；本地 `Spawn` 与远端收包统一走 `INetworkObjectFactory`，本地 `Spawn` 接收已创建实例（创建/对象池在外部），远端收包经 `INetworkObjectFactory.Create`；`SpawnLocal`/`DestroyLocal` 只做绑定与解绑。状态为 `NetworkVariableBase` 列表（由 `NetworkObject.Register`/`Unregister` 登记，按下标同步；`IsDirty` 一帧内多次置位只在 `Flush` 时发最终值，并进入 Spawn/晚加入快照）。`NetworkObject.Serialize`/`Deserialize` 接在快照变量列表之后，仅 Spawn/晚加入，不参与 Flush。RPC 为对象通道 `Register`/`SendToOthers`（每次立即发送）。快照按列表顺序由各变量 `Write`/`Read` 再接对象 `Serialize`/`Deserialize` 成对推进同一条流（无 length 分帧）；State 为 `Sequence + index + payload`（类型 4），Rpc 为 `Sequence + channel + payload`（类型 5）。位姿在 Awake 登记一次，不随 Bind/Unbind 插拔。PrefabId 为 `int`（`Animator.StringToHash(prefab.name)`，0 保留，注册冲突断言；YooAsset 预制体名全局唯一）。`NetworkObjectId(PeerId, Sequence)`，载荷只传 Sequence；Spawn/Despawn 为类型 2/3，状态上限 1024。会话信封按 `PayloadCapacity` 分配，不感知 Spawn 头。对象 RPC/State 与 Spawn/Despawn 发送缓冲走 `ArrayPool`，不按对象常驻。未在 Unity 编译/运行；Loopback 样例会话 API 仍待迁，游戏项目接线仍待办。
